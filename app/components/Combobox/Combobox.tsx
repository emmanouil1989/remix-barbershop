...
export function ComboBox<T extends ComboBoxValue>({
  ...
  return (
    <ReactAriaCombobox {...props}>
      <Label>{label}</Label>
      <Group className="flex rounded-lg bg-white bg-opacity-90 focus-within:bg-opacity-100 transition shadow-md  outline-none">
...
export function ComboboxItem({ ...props }: ListBoxItemProps) {
  return (
    <ListBoxItem
      {...props}
      className="flex flex-row items-center gap-4 outline-none w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-400 dark:hover:text-white border-b border-solid border-gray-600 last:border-b-0 selected:font-bold"
    />
  );
}
